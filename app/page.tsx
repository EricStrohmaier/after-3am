...
export default function Home() {
  ...
  const [devMode, setDevMode] = useState(() => {
    // Check if we have a saved dev mode preference
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between bg-black text-white">
      ...
      <div className="w-full max-w-3xl p-4 flex flex-col items-center justify-center min-h-screen">
        {isWithinTimeGate ? (
          <>
            <h1 className="text-2xl font-light mb-8 text-center opacity-70">Ask Me After 3AM</h1>

            <ModeSelector currentMode={mode} setMode={setMode} />

            <QuestionInput mode={mode} setQuestion={setQuestion} setResponse={setResponse} setIsTyping={setIsTyping} />

            <ResponseDisplay response={response} isTyping={isTyping} audioEnabled={audioEnabled} />
          </>
        ) : (
